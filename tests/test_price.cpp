...
TEST_P(PriceParseInvalid, Invalid) {
    ...
    EXPECT_FALSE(P::parse(tc.input).has_value())
        << "ERROR: Expected no value. "
        << "Input: " << tc.input;
}
...
TEST_P(PriceParseValid, Valid) {
	...
	EXPECT_EQ(p->ticks(), tc.expectedTicks)
        ...
        << "Parsed Ticks: " << p->ticks()
        << " Expected Ticks: " << tc.expectedTicks;
...
INSTANTIATE_TEST_SUITE_P(
    ...
    PriceParseValid,
    ::testing::Values(
        ValidParseData{ "ParsesInteger", "123", 1230000},
